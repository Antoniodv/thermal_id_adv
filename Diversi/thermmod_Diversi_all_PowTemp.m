% In this script the input to the identification are partial powers and
% temperatures
...
for ind_node = 1%:num_nodpkg
    ...
    temperatures_toest = temperatures(2:end-1,:);


    % Calculate parameters and noise matrices in all windows
    num_autocorr = 100;
    theta_init = []; 
    num_input = num_cores+1+num_cores-1;
    %num_input = num_cores-1;
    theta = NaN((1+num_input)*model_order, num_cores, num_win);
    ...
    svd_ident = NaN((1+num_input)*model_order, num_cores, num_win);
    for core = 1:num_cores
        pow_touse = [pow_part(3:end,:), temperatures(1:end-2, 1+setdiff(1:num_cores,core))];
        %pow_touse = temperatures(1:end-2, 1+setdiff(1:num_cores,core));
        ...
        for ind_win = 1:num_win
            ...
            [theta(:,core,ind_win), sigv(core,ind_win), sigw(core,ind_win), res, ...
                ...
                misoarxbls(pow_touse(ind_curr,:), temperatures_toest(ind_curr, 1+core), model_order, theta_init);
    ...
    Ts = 2;
    T_pred = NaN(size(temperatures_toest,1),num_cores,num_win);
    T_filt = T_pred;
    residuals = T_pred;
    for core = 1:num_cores
        pow_touse = [pow_part(3:end,:), temperatures(1:end-2, 1+setdiff(1:num_cores,core))];
        ...
        for ind_win = 1:num_win
            A = [-theta(1:model_order,core,ind_win), [eye(model_order-1); zeros(1,model_order-1)]];
            B = reshape(theta(model_order+1:end,core,ind_win), model_order, num_input);
        ...
        %     y = lsim(sys, pow_part_re, 0:Ts:Ts*(size(temperatures,1)-1), Xi);
            ...
            for ind_win_Kalman = 1:num_win
                ...
                [Xi, Pi, zf, yk] = KalmanFilter(A, eye(model_order), C, -A*Pi*A.' + Pi, R, ...
                    Xi, Pi, temperatures_toest(ind_curr(1:model_order),1+core), temperatures_toest(ind_time,1+core));
                T_pred(ind_time,core,ind_win) = yk(1) + temperatures_toest(ind_time,1+core);
                T_filt(ind_time,core,ind_win) = zf;
                residuals(ind_time,core) = yk(1);
                for ind_time = ind_curr(2):ind_curr(end)
                    [Xi, Pi, zf, yk] = KalmanFilter(A, B, C, Q, R, Xi, Pi, pow_touse(ind_time-1,:)', temperatures_toest(ind_time,1+core));
                    T_pred(ind_time,core,ind_win) = yk(1) + temperatures_toest(ind_time,1+core);
                    T_filt(ind_time,core,ind_win) = zf;
                    residuals(ind_time,core) = yk(1);
                end
    ...
    temperatures_toest = (temperatures_toest + temp_mean) + data.temp_amb(3:end-1)' * temp_std;
    temperatures_all = temperatures_toest;
    T_pred = (T_pred + temp_mean(2:end)) + data.temp_amb(3:end-1)' * temp_std;
    T_filt = (T_filt + temp_mean(2:end)) + data.temp_amb(3:end-1)' * temp_std;
    err_pred = temperatures_toest(:,2:9) - T_pred;
    err_filt = temperatures_toest(:,2:9) - T_filt;
    
    % Save results
    save(out_path + "KalmanIdentAll_WinSep_PowTemp_" + nodespkg(ind_node) + ".mat", ...
        'T_pred', 'eigenvalues', 'nodespkg', 'temperatures_all', 'resid_autocorr', ...
        'sigv', 'sigw', 'theta', 'cond_ident', 'svd_ident') %, '-v7.3')
%     save(out_path + "KalmanIdentAll_WinSep_Temp_" + nodespkg(ind_node) + ".mat", ...
%         'T_pred', 'eigenvalues', 'nodespkg', 'temperatures_all', 'resid_autocorr', ...
%         'sigv', 'sigw', 'theta', 'cond_ident', 'svd_ident') %, '-v7.3')
