...
% Perform the identification with all data in the training set

num_nodwin = num_nodpkg * num_cores * num_win;
win_length = win_length_min;

num_sub = 20;
theta_all = NaN(10*model_order, num_sub);
...
for ind_sub = 1:num_sub
    ...
    pow_touse = NaN(length(nodwin_train_sub) * win_length, 1+num_cores);
    ind_wintrain = 0;
...
for ind_node = 1:num_nodpkg
    ...
    for core = 1:num_cores
        ...
        B = reshape(theta(model_order+1:end), model_order, 1+num_cores);
    ...
    % Save results
    save(out_path + "KalmanIdentAllCombinedPowPart_" + nodespkg(ind_node) + ".mat", ...
