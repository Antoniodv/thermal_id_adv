    ...
    for k = 1:length(unique_paths)
        ...
        try
            ...
            if exist(pkl_file, 'file')
                if strcmp(mode, "all")
                    [tmp_coeff_matrix, active_cores_indexes] = thermal_model_est_mocked(pkl_file, "./", thermal_model_path, mode);
                    global_matrix(:, :, k) = tmp_coeff_matrix;
                    active_cores_indexes_matrix(k,:) = active_cores_indexes;

                elseif strcmp(mode, "patch")    
                    disp("patch")
                    [tmp_coeff_matrix, active_cores_indexes] = thermal_model_est_mocked(pkl_file, path_to_active_cores, thermal_model_path, mode);
                    global_matrix(:, :, k) = tmp_coeff_matrix;
                    active_cores_indexes_matrix(k,:) = active_cores_indexes;
